...
def home():
    ...
    if request.method == 'POST':
        ...
        current_step = int(request.form.get('step', state["step"]))  # Получаем шаг из формы
        state["step"] = current_step  # Обновляем шаг

        print(f"User: {user_id}, Step: {state['step']}, Choice: {choice}")  # Отладка
